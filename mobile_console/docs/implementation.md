...
```
mobile_console/
├── run.py                 # 控制台全部代码(后端 + 内嵌前端单页),~1247 行
...
### `ClaudeRunner`（run.py:208）

Claude 的任务队列模型。**不继承 ManagedProcess**（因为它不是长驻进程，而是「逐条调 claude -p」）。

关键方法：
- `start()` — 启动后台 `_work_loop` 线程。
- `submit(prompt)` — 入 `task_q` 队列，worker 逐条出队处理。
- `interrupt()` — 杀当前 claude 子进程整个进程组 + 清空待办队列（等同按 Esc）。会设 `_interrupted` 标志，让 `_run_one` 不报「异常退出」。
- `_work_loop()` — `while self._started`：从 `task_q` 取一条 → `_push(prompt, "you")` 回显 → `_run_one(prompt)`。
- `_run_one(prompt)` — 组装并跑 `claude -p [--continue] --dangerously-skip-permissions`，prompt 走 stdin。逐行读 stdout 推队列。首条成功后 `_has_session=True`，之后带 `--continue`。**详见下方「_run_one 平台分流」。**
- `_append_log(tag, line)` — **持久化日志**。按天写 `logs/claude-YYYY-MM-DD.log`，多行内容每行带 `HH:MM:SS [tag]` 前缀，续行对齐。`_log_lock` 保证线程安全。失败静默（不影响主流程）。
- `_push(line, tag)` — **双写**：入内存队列（推手机）+ 调 `_append_log`（落盘）。

> 注意：`ManagedProcess._push` 和 `ClaudeRunner._push` 是两个独立方法。**只有 Claude 的会落盘**，Flask 的不会。

### `_run_one` 的平台分流（run.py:356，核心）

**Windows 分支**：
```python
cmd = [claude_bin, "-p", ("--continue",), "--dangerously-skip-permissions"]
# shell=True 仅当 claude 是 .cmd/.bat
Popen(cwd=PROJECT_ROOT, stdin=PIPE, creationflags=CREATE_NEW_PROCESS_GROUP, ...)
```

**Linux 分支**（关键，踩坑后的最终形态）：
```python
...
为什么要 `runuser -l console` + `source ~/.claude.env`：
1. root 下 claude 2.1+ 拒绝 `--dangerously-skip-permissions` → 必须降权到 console 用户。
2. ANTHROPIC 凭据（`ANTHROPIC_AUTH_TOKEN` + `ANTHROPIC_BASE_URL` 等）存在 `/home/console/.claude.env`，由 console 用户的 `~` 解析。控制台进程本身的环境不需要这些变量。
3. `runuser -l` 是 login shell，会 `cd` 到家目录覆盖 Popen 的 cwd → 必须在 shell_cmd 里显式 `cd {PROJECT_ROOT}`，否则 claude 以为工作目录是 `/home/console`，看不到项目代码、trust 也不匹配。

prompt 传递：`proc.stdin.write(prompt); proc.stdin.close()` → claude 收 EOF 开始处理。

## 全局实例与 PROCS 字典（run.py:448）
...
`start_flask()` 组装 gunicorn 命令调 `flask_proc.start()`；`start_claude()` 调 `claude_proc.start()`。
...
| POST | `/api/start/<key>` | 登录 | key=claude/flask，启动 |
| POST | `/api/startall` | 登录 | 一键启 Claude+Flask |
| POST | `/api/stop/<key>` | 登录 | 停止 |
| POST | `/api/restart/<key>` | 登录 | 重启 |
| POST | `/api/send/<key>` | 登录 | key=claude 入队；key=flask 投 stdin |
| POST | `/api/interrupt` | 登录 | 打断当前 claude 指令 |
...
- **`/api/peek`**（run.py，免登录）：复用 `PROCS` 的 `status()`，返回 `console` / `claude` / `flask` 各自的 running/pid/端口占用 + `lan_ip`。`_port_listening(port)` 用 `socket.connect_ex` 探测端口。`@app.before_request` 放行 `/api/peek` 和 `/status`。
- **`/status`**（run.py `status_page`）：免登录，从磁盘读 `status.html` 返回。
- **`status.html`**：纯静态前端，3s 轮询 `/api/peek`，状态灯（绿=运行、橙=处理中/端口占用、灰=未运行）。Claude busy 时顶部弹冲突预警条。`peekUrl()` 支持 `?host=` 参数连别的地址，否则同源，兜底 `127.0.0.1:8765`（支持 `file://` 直接打开）。
...
生成器 `stream()` 轮询所有 `PROCS` 的 `log_q`，有就 `yield` 一条 JSON（`{ts, tag, line}`），每 ~15s 发心跳防代理断连。前端 `EventSource('/api/logs')` 自动带同源 cookie，`onmessage` 按 `tag` 分流到 `log-all` / `log-claude` / `log-flask` 三个终端区。

## 前端（PAGE_HTML，run.py:833）

单页 + 原生 JS，无框架。三个 tab：控制台总览 / Claude / Flask。关键行为：
- `connectSSE()`：建 SSE，`onerror` 1.5s 后重连（除非已被踢）。
- `refreshStatus()`：每 3s 轮询 `/api/status`，更新状态灯 + busy 横幅。
- `send('claude')`：把已载入的文件上下文块（`_fileBlocks`）拼在指令前一起发，发送后清空。
- `onFilesPicked()`：逐个上传文件，服务端返回上下文块，前端累积成 chip，可单独移除。
- `onKicked()`：被踢时弹全屏遮罩，`_kickedShown` 保证只弹一次。

## 持久化日志格式（`logs/claude-YYYY-MM-DD.log`）
...
tag 缩写：`you` / `claude` / `sys`(system) / `ERR`(error)。多行回复续行对齐（缩进等于时间戳+标签宽度），方便 `grep` / `tail` 时行行可定位。

## 改代码时的常见入口

| 想改 | 改哪里 |
|------|--------|
| claude 调用参数 | `ClaudeRunner._run_one`（run.py:356） |
| 恢复逐项授权 | 去掉 `_run_one` 里 `--dangerously-skip-permissions` |
| 持久化更多东西 | 在 `ClaudeRunner._push` 里已是双写；要给 Flask 也落盘就改 `ManagedProcess._push` 加 `_append_log` |
