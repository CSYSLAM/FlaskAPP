...
```
任一 AI（Claude / CodeBuddy）处理中时顶部会醒目提示「有 AI 正在处理指令，请勿在电脑上同时改代码」。
...
1. 进主页，点 **「⚡ 一键启动 Claude + CodeBuddy + Flask」**（或分别点启动）。
2. 切到 **Claude** 或 **CodeBuddy** tab → 在输入框发指令（回车换行，点「发送」或 `Ctrl+Enter` 提交）。
3. 对应 AI 的输出实时回流到各自 tab 的日志区。处理中时顶部有横幅，可点该 tab 的 **「✋ 打断」** 中止当前指令（等同按 Esc，不影响会话记忆）。
4. 切到 **Flask** tab → 点「▶ 打开游戏页面」直达 `http://117.72.209.69:5000`。
5. 报错实时看 **实时日志** 区，可贴给 AI 让它修。

> Claude 与 CodeBuddy 是两个独立 agent，可同时开启：Claude 降权到 `console` 用户跑，CodeBuddy 以当前用户（root）直接跑，互不干扰。指令/文件上下文按 agent 隔离，不会串。

### 4. 电脑上看手机发过的指令历史

手机发的每条指令 + AI 回复都落盘了（服务重启不丢）：
...
# 看今天的 CodeBuddy 日志
cat /mnt/FlaskAPP/mobile_console/logs/codebuddy-$(date +%F).log

# 实时跟踪(两个一起)
tail -f /mnt/FlaskAPP/mobile_console/logs/claude-$(date +%F).log \
       /mnt/FlaskAPP/mobile_console/logs/codebuddy-$(date +%F).log

# 看有哪些天的日志
ls /mnt/FlaskAPP/mobile_console/logs/

# 搜关键词
grep '你搜的词' /mnt/FlaskAPP/mobile_console/logs/claude-*.log /mnt/FlaskAPP/mobile_console/logs/codebuddy-*.log
```
...
| 启动/停止/重启 CodeBuddy | 控制台 tab 或 CodeBuddy tab 的按钮 |
| 启动/停止/重启 Flask | 控制台 tab 或 Flask tab 的按钮 |
| 一键启动 | 主页顶部「⚡ 一键启动 Claude + CodeBuddy + Flask」 |
| 发指令给 Claude / CodeBuddy | 对应 tab 输入框 → 发送 |
| 打断当前指令 | 对应 tab「✋ 打断」 |
| 附文件作上下文 | 对应 tab「📎 附文件」选 `.txt`/`.md`（≤256KB），发送时拼在指令前（按 agent 隔离） |
...
**Q: 想恢复 Claude / CodeBuddy 逐项授权（不要无人值守）？**
A: 去掉 `AgentRunner._run_one`（run.py:368）里 `cli_args.append(self.skip_flag)`（即 `--dangerously-skip-permissions`）。但手机端没法点确认，会卡住，仅在你改用别的交互方式时才合适。

**Q: 同时开 Claude 和 CodeBuddy，其中一个失效 / 报 `claude.env` 相关错误？**
A: 这是 `env_file` 路径解析问题（详见 [踩坑文档坑13](pitfalls.md#坑-13env_file-路径解析错导致-source-失败)）。确认 `_run_one` 里 `env_file` 是按降权用户家目录拼成的绝对路径 `/home/console/.claude.env`，而不是被 Python `expanduser()` 成 `/root/.claude.env`、或被 `shlex.quote` 包成 `'~/.claude.env'`。CodeBuddy 本身不需要降权，不会触发此问题。
