...
手机控制台是一个**独立的 Flask 服务（端口 8765）**，让你在手机浏览器上：驱动本机 Claude Code 改代码 + 启停游戏进程 + 实时看日志。它和游戏 app（`app.py`，端口 5000）是两个完全独立的进程，**不 import 任何游戏模块**，可以单独删除/移动而不影响游戏。
...
### 1. Claude 用「任务队列」而非长驻进程

`claude` CLI 有两种模式：
- **默认交互模式**：要 TTY。手机网页是非 TTY，用不了。
- **`-p`（print）模式**：一次性。每条指令跑完进程就退出。

没法让一个 claude 进程常驻着、手机往里灌指令。所以采用**任务队列 + 逐条调用**模型：

```
手机发指令 → POST /api/send/claude → 入 task_q 队列
                                      ↓
                          后台 _work_loop 线程逐条出队
                                      ↓
        subprocess: claude -p [--continue] --dangerously-skip-permissions
        (prompt 从 stdin 投递，写完立即 close → claude 收 EOF 开始处理)
                                      ↓
        逐行读 stdout → push 到 log_q → SSE 实时推给手机
```

- 首条指令不带 `--continue`（没会话可续），之后每条带 `--continue` 接续同一会话，**Claude 记得前文**（记忆链靠 claude CLI 自己的会话存储，在 `~/.claude/` 下，不是控制台管的）。
...
### 4. Linux 下 claude 必须降权到非 root 用户

claude CLI 2.1+ 拒绝以 root 跑 `--dangerously-skip-permissions`（报 `cannot be used with root/sudo privileges`）。但控制台本身常以 root 跑（服务器/systemd）。所以 Linux 下调 claude 用 `runuser -l console -c '...'` 切到 `console` 用户执行，ANTHROPIC 凭据从 `/home/console/.claude.env` 加载。详见 [踩坑文档](pitfalls.md)。
...
所以 Claude 交互（`ClaudeRunner._push`）做**双写**：入内存队列（推手机）+ 追加到 `mobile_console/logs/claude-YYYY-MM-DD.log`（电脑可 `tail`/`cat`，重启不丢）。Flask 进程的日志只入内存队列（不持久化），因为它有自己的 gunicorn 日志。
...
- `/status`：配套的纯静态看板页（`status.html`），前端轮询 `/api/peek`，3s 刷新，有状态灯。Claude 处理中时醒目提示「设计者正在用手机操作，请勿在电脑上同时改代码」。
